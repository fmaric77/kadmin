  ...
  <q-page class="q-pa-md">
    <q-card>
      ...
      <q-card-section>
        ...
        <q-btn
          ...
          label="Dodaj Narudžbu"
          color="primary"
          class="q-mb-md"
        />
        <q-btn
          @click="exportOrdersToPDF"
          label="Izvezi narudžbe"
          color="secondary"
          class="q-mb-md"
        />
        <q-table
        ...
        >
          ...
          <template v-slot:body-cell-Kupac="props">
            <q-td :props="props">{{
              getCustomerName(props.row.SIFRA_KUPCA)
            }}</q-td>
          </template>
          <template v-slot:body-cell-Prodavac="props">
            <q-td :props="props">{{
              getSellerName(props.row.SIFRA_PRODAVACA)
            }}</q-td>
          </template>
          <template v-slot:body-cell-Datum_narudzbe="props">
            <q-td :props="props">{{
              formatDate(props.row.Datum_narudzbe)
            }}</q-td>
          </template>
          ...
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" align="center">
              <q-btn
                ...
                icon="delete"
                color="red"
                @click="deleteOrder(props.row.Sifra_narudzbe)"
              />
            </q-td>
          </template>
        </q-table>
    ...
    <q-dialog v-model="isDialogOpen">
      <q-card>
        <q-card-section>
          <div class="text-h6">
            {{ isEditing ? "Uredi Narudžbu" : "Dodaj Narudžbu" }}
          </div>
        </q-card-section>
        <q-card-section>
...
const columns = [
  ...
  {
    name: "Cijena_narudzbe",
    label: "Cijena narudžbe",
    field: "Cijena_narudzbe",
    align: "left",
  },
...
const updateOrder = async () => {
  ...
  try {
    ...
    await axios.put(
      `${baseURL}/orders/${form.value.Sifra_narudzbe}`,
      orderData
    );
    fetchOrders();
    closeDialog();
...
defineExpose({ showAddOrderDialog, closeDialog });
</script>
