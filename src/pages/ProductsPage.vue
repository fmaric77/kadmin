      ...
      <q-card-section>
        ...
        >
          <template v-slot:body-cell-Naziv="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingNaziv"
                @dblclick="enableEditing(props.row, 'naziv')"
                @touchstart="enableEditing(props.row, 'naziv')"
              >
                {{ props.row.Naziv }}
              </div>
              <q-input
                ...
                @blur="saveChanges(props.row, 'naziv')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-Marka="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingMarka"
                @dblclick="enableEditing(props.row, 'marka')"
                @touchstart="enableEditing(props.row, 'marka')"
              >
                {{ props.row.Marka }}
              </div>
              <q-input
                v-else
                v-model="props.row.Marka"
                @blur="saveChanges(props.row, 'marka')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-Cijena_proizvoda="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingCijena"
                @dblclick="enableEditing(props.row, 'cijena')"
                @touchstart="enableEditing(props.row, 'cijena')"
              >
              ...
              <q-input
                v-else
                type="number"
                v-model="props.row.Cijena_proizvoda"
                @blur="saveChanges(props.row, 'cijena')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" align="center">
              <q-btn
                ...
                icon="delete"
                color="red"
                @click="deleteProduct(props.row.Sifra_proizvoda)"
              />
            </q-td>
          </template>
        </q-table>
...
const updateProduct = async () => {
  await axios.put(
    `${baseURL}/products/${form.value.Sifra_proizvoda}`,
    form.value
  );
  fetchProducts();
  closeDialog();
