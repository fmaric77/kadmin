  ...
  <q-page class="q-pa-md">
      ...
      <q-card-section>
        ...
        >
          <template v-slot:body-cell-Ime_kupca="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingName"
                @dblclick="enableEditing(props.row, 'name')"
                @touchstart="enableEditing(props.row, 'name')"
              >
                {{ props.row.Ime_kupca }}
              </div>
              <q-input
                v-else
                v-model="props.row.Ime_kupca"
                @blur="saveChanges(props.row, 'name')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-Prezime_kupca="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingSurname"
                @dblclick="enableEditing(props.row, 'surname')"
                @touchstart="enableEditing(props.row, 'surname')"
              >
                {{ props.row.Prezime_kupca }}
              </div>
              <q-input
                v-else
                v-model="props.row.Prezime_kupca"
                @blur="saveChanges(props.row, 'surname')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-Adresa_kupca="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingAddress"
                @dblclick="enableEditing(props.row, 'address')"
                @touchstart="enableEditing(props.row, 'address')"
              >
                {{ props.row.Adresa_kupca }}
              </div>
              <q-input
                v-else
                v-model="props.row.Adresa_kupca"
                @blur="saveChanges(props.row, 'address')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-E_mail="props">
            <q-td :props="props">
              <div
                v-if="!props.row.isEditingEmail"
                @dblclick="enableEditing(props.row, 'email')"
                @touchstart="enableEditing(props.row, 'email')"
              >
                {{ props.row.E_mail }}
              </div>
              <q-input
                v-else
                v-model="props.row.E_mail"
                @blur="saveChanges(props.row, 'email')"
              />
            </q-td>
          </template>
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" align="center">
              <q-btn
                ...
                icon="delete"
                color="red"
                @click="deleteCustomer(props.row.Sifra_kupca)"
              />
            </q-td>
          </template>
        </q-table>
    ...
    <q-dialog v-model="isDialogOpen">
      <q-card>
        <q-card-section>
          <div class="text-h6">
            {{ isEditing ? "Uredi Kupca" : "Dodaj Kupca" }}
          </div>
        </q-card-section>
        <q-card-section>
          <q-form
            @submit.prevent="isEditing ? updateCustomer() : addCustomer()"
          >
            <q-input filled v-model="form.Ime_kupca" label="Ime" required />
            <q-input
              filled
              v-model="form.Prezime_kupca"
              label="Prezime"
              required
            />
